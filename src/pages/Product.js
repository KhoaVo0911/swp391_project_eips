...
import {
  PostComboProductAsyncApi,
  PostProductAsyncApi,
  PostRevenueAsyncApi,
  ProductAction,
  PutProductAsyncApi,
  getAllProductAsyncApi,
  getProductAsyncApi,
  getProductSoldAsyncApi,
...
function Product() {
  ...
  const { ProductListAll, ProductSold, Revenue } = useSelector(
    (state) => state.product
  );
  ...
  const handleClickOpenOrder = () => {
    setOpenOrder(true);
    dispatch(GetListOrderAsyncApi(shopByUsername.id))
      .then((response) => {
    ...
    dispatch(getShopByUsernameAsyncApi(userObject.username))
      .then((response) => {
        if (response.payload != undefined) {
          ...
          dispatch(getAllProductAsyncApi(response.payload.id))
            .then((response) => {
              if (response.payload != undefined) {
            ...
            .catch((error) => {});
          dispatch(getProductSoldAsyncApi(response.payload.id))
            .then((response) => {
              if (response.payload != undefined) {
              }
            })
      ...
      uploadTask.on(
        ...
        () => {
          getDownloadURL(uploadTask.snapshot.ref).then(async (downloadURL) => {
            ...
            const newDataBody = [DataBody[0], ...DataBody[1].flat()];

            dispatch(PostComboProductAsyncApi(newDataBody))
              .then((response) => {
                setComboList([]);
                setNameCombo("");
                setImageCombo("");
                ...
                if (response.payload != undefined) {
                  ...
                  dispatch(getAllProductAsyncApi(shopByUsername.id))
                    .then((response) => {
                      if (response.payload != undefined) {
                        setFilteredData(response.payload);
                      }
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      if (click == false) {
        ...
        if (isUpdate == false) {
          dispatch(PostProductAsyncApi(DataBody))
            .then((response) => {
              setImg("");
              SetClick(false);
              setSelectedImage();
              ...
              if (response.payload != undefined) {
                // dispatch(getAllProductAsyncApi({ shopId: shopByUsername.id })).then((response) => {
                dispatch(getAllProductAsyncApi(shopByUsername))
                  .then((response) => {
                    if (response.payload != undefined) {
                      setFilteredData(response.payload);
                    }
          ...
          dispatch(PutProductAsyncApi(DataBodyUpdate))
            .then((response) => {
              setImg("");
              SetClick(false);
              setSelectedImage();
                          ...
                          filteredData.map((products, index) => (
                            <tr key={products.id}>
                              ...
                              <td itemScope="row" className="cursor-pointer">
                                ...
                                ) : products.category == "Combo" ? null : (
                                  <Tooltip
                                    onClick={() =>
                                      handleClickAddCombo(products)
                                    }
                                    title="Add Combo"
                                  >
                                    <IconButton>
                ...
                <div className="col-lg-6 col-6" style={{ paddingTop: "145px" }}>
                  <div className="custom-block bg-white">
                    ...
                    <div className="mb-2 float-right">
                      <p className="text-xl uppercase font-bold mt-[10px]">
                  ...
                  <div className="custom-block bg-white">
                    ...
                    <form>
                          ...
                          <tr style={{ textAlign: "center" }}>
                            ...
                            <td scope="row" className="mx-2 px-2">
                              <input
                                ...
                                onChange={(e) =>
                                  setDescriptionCombo(e.target.value)
                                }
                              />
                            </td>
                            <td scope="row" className="mx-2 px-2">
                      ...
                      <div className="float-right">
                        <Button
                          disabled={comboList.length > 0 ? false : true}
                          onClick={handleClickSubmitCombo}
                          color="primary"
                          variant="contained"
                        >
                          Submit
          ...
          <DialogContent dividers>
            ...
            <div className="max-w-5xl my-2 mx-auto">
              <TextField
                id="outlined-basic"
                error={
                  formik.touched.price && formik.errors.price ? true : undefined
                }
