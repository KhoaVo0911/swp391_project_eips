...
export default function Cart() {
  ...
  const handleCheckout = () => {
    ...
    const newDataBody = [body[0], ...body[1].flat()];
    dispatch(PostOrderAsyncApi(newDataBody))
      .then((response) => {
        if (response.payload != undefined) {
  ...
  const handleIncrease = (item) => {
    dispatch(
      cartAction.increaseQuantityTocart({
        quantity: item?.quantity,
        name: item?.name,
        price: item?.price,
        category: item?.category,
  ...
  const handleDecrease = (item) => {
    if (item.quantity > 1) {
      dispatch(
        cartAction.decreaseQuantityTocart({
          quantity: item?.quantity,
          name: item?.name,
          price: item?.price,
          category: item?.category,
              ...
              >
                ...
                <tbody>
                  <tr>
                    ...
                    <th scope="col">
                      {SearchCardList.status == true
                        ? "true"
                        : SearchCardList.status == false
                        ? "false"
                        : null}
                    </th>
                  </tr>
              ...
              return (
                ...
                >
                  ...
                  <span className="text-center w-1/5 font-semibold text-sm">
                    {parseToVND(item.price)} Vnđ
                  </span>
                  <span className="text-center w-1/5 font-semibold text-sm">
                    {parseToVND(item.price * item.quantity)} Vnđ
                  </span>
                </div>
              );
            })}
          ...
          <div id="summary" className="w-1/4 px-8 py-10">
            ...
            <div className="flex justify-between mt-10 mb-5">
              <span className="font-semibold text-sm uppercase">
                {arrCart.length} Items
              </span>
              <span className="font-semibold text-sm">
                {parseToVND(total)} Vnđ
              </span>
            </div>
            <div></div>
            ...
            <div className="border-t mt-8">
              ...
              <button
                disabled={SearchCardList.balance < total ? true : false}
