              ...
              if (response.payload != undefined) {
                const updatedArr = response.payload.map((item) => {
                  return {
                    ...
                    quantity: 1, // Thêm thuộc tính mới
                  };
                });
          ...
          {filteredData &&
            filteredData.map((product, index) => (
              <div key={index} className="  border-2 p-2 ">
                <div className="w-full overflow-hidden rounded-md bg-gray-200 group-hover:opacity-75 lg:h-80 flex justify-center items-center">
                  <img
                    src={product.img}
